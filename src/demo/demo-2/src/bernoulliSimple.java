// Creates a random graph (Bernoulli) and undergo adoption process in Simple way. Writes all graph data to single files for line plot (timestepData) 
...
public class bernoulliSimple
{   
    public static void main(String [] args) throws ExportException, IOException
    {
    	File timestepData = new File("../../../data/timestepDataBernoulliSimp.csv"); // file to record timestep data for plotting simple line graph
    	File linearEqs = new File("../../../data/linearEqsBernoulliSimp.csv"); // file to be read by python to calculate p and q. Includes S(t+1) data, and coefficients of a, b, c
	...
	private static void createAndAdopt(FileWriter timestepfw,
        ...
        graphState state = new graphState();
     	...
     	do {
     		...
     		if (!state.getIntAdoptionHappen()) {
       			graph = externalAdoption(graph,ni,p,state);			
     		}
         	//exportCSV files
